...
8. **Word-level verbatim ASR only.** Never SRT/phrase mode (loses sub-second gap data). Never normalized fillers (loses editorial signal) — `transcribe.py --verbatim` primes Whisper to include them; without it, "no filler words found" proves nothing.
9. **Cache transcripts per source.** Never re-transcribe unless the source file itself changed. **Verify the cache actually reflects what you asked for before trusting it** — e.g. if you requested `--verbatim`, open the transcript JSON and check its `"verbatim"` field is `true` before claiming a verbatim pass happened. A background job that silently dies leaves the old cached file untouched; treating that stale file as fresh is how a real incident produced a confident but false "no fillers detected" claim (2026-09-07).
